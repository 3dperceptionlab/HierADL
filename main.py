...
def get_config():
    ...
    # Set wandb
    wandb.require('core')
    wandb.init(project="AutomaticHierarchies",
...
def main():
    ...
    # Get dataset
    train_ds = HierarchicalDataset(config, split="train")
    test_ds = HierarchicalDataset(config, split="test")
    ...
    # Create the model
    model = AggregationTransformer(config, train_ds)
    model = model.cuda()
    ...
    for epoch in range(config.solver.epochs):
        ...
        for batch,(rgb_t, coarse_target, fine_target) in enumerate(tqdm(train_loader)):
        ...
        if epoch % config.solver.eval_freq == 0:
            ...
            res = evaluate(epoch, model, test_loader, config, coarse_criterion, fine_criterion, train_ds.fine_only, train_ds.num_coarse_classes, train_ds.num_fine_classes)
